/*
    ...
    (무작정 다 조회해 오는게 아니라 각 테이블간 연결고리(외래키)를 통해 데이터를 매칭시켜 조회
    
    JOIN은 크게 "오르클 전용구문" "ANSI구문"
...
--JOIN절에 같이 조인하고자하느 테이블 기술 + 매칭시킬 컬럼에대한 조건
--JOIN USING / JOIN ON

--1. 연결할 두 컬럼명이 다른 경우(EMPLOYEE : DEPT_CODE, DEPARTMENT : DEPT_IP)
-- JOIN ON
--전체 사원들의 사번, 사원명, 부서코드, 부서명
...
ORDER BY DEPT_CODE NULLS FIRST, DEPT_ID NULLS FIRST;
...
ORDER BY SAL_LEVEL DESC, SALARY, EMP_NAME;
