/*
    ...
    임시테이블 같은 존재(실제 데이터가 담겨있는 건 아님 -> 논리테이블)
*/
...
--CREATE OR REPLACE를 사용하면 VIEW가 없을때는 생성, 이미 존재한다면 수정할 수 있다.
...
/*
    ...
    2) FORCE | NOFORCE
        ...
        > NOFORCE : 서브쿼리에 기술된 테이블이 존재하는 테이블이어야만 한다 (기본값)
    3) WITH CHECK OPTION : DML시 서브쿼리에 기술된 조건에 부합한 값으로만 DML이 가능하도록
    4) WITHC READ ONLY : 뷰에 대해서 조회만 가능하도록
*/

--2) FORCE | NOFORCE
CREATE OR REPLACE NOFORCE VIEW VM_EMP
AS  SELECT TCODE, TCONTENT
    FROM TT;
