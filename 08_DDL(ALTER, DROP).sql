...
--DEFALUT값 수정 : MODIFY 컬럼명 DEFAULT 변경할기본값
 ...
 --1_3) 컬럼 삭제(DROP COLUMN) : DROP COLUMN 삭제하고자하는컬럼
...
ALTER TABLE DEPT_CODY DROP COLUMN CNAME;
 ...
 ALTER TABLE DEPT_CODY DROP COLUMN LOCATION_ID; --테이블은 최소 하나이상의 컬럼이 필요
 
 -----------------------------------------------------------------------
 --2) 제약조건 수차 / 삭제(수정은 삭제하고 다시 추가하면 됨.
...
-------------------------------------------------------------------
--3) 컬럼명/제약조건명/테이블명 변경
--3_1) 컬럼명 변경 : RNAME COLUMN 기존컬러명 TO 바꿀 컬러명
