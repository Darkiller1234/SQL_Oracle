...
-----------------------------------------------------------------------
--1_2) 래퍼런스 타입 변수 선언및 초기화(어떤 테이블의 어떤 컬럼의 데이터타입을 참조해서
--                                  그 타입으로 지정)
...
사용자가 입력한 사번의 사원의 사번, 사원명, 직급코드 급여 부서면 조회 후 각 변수에 담아 출력 */
...
-------------------------실습-------------------------
--DECLARE
--래퍼런스변수 (EID, ENAME, ETITLE, NCODE)
--      참조변수 (EMP_ID, EMP_NAME, DEPT_TITLE, NATIONAL_CODE)
--일반타입변수 ( THEN 문자열) - 국내팀, 해외팀 분리해서 저장
--BEGIN
--사용자가 입력한 사번의 사원정보를 가져와 사번, 이름, 부서면, 근무국가코드 조회 후 각 변수에 대입
...
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    ETITLE DEPARTMENT.DEPT_TITLE%TYPE;
    NCODE LOCATION.NATIONAL_CODE%TYPE;
    TEAM VARCHAR2(10);
BEGIN
    SELECT EMP_ID, EMP_NAME, DEPT_TITLE, NATIONAL_CODE
    INTO EID, ENAME, ETITLE, NCODE
    FROM EMPLOYEE
    JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
    ...
    DBMS_OUTPUT.PUT_LINE('부서 : ' || ETITLE);
...
/*
    ...
    EXCEPTION
        WHEM 예외명1 THEN 처리구문1;
        WHEM 예외명2 THEN 처리구문1;
        ...
        
    *시스템 예외(오라클에서 미리 정의해둔 예회)
    -NO_DATE_FOUND : SELECT한 결과가 한행도 없을 때
    -TOO_MANY_ROWS : SELECT한 결과가 여러행일 경우
    -ZERO_DIVIDE : 0으로 나눌 때
    -DUP_VAL_ON INDEX : UNIQUE 제약조건 위배
    ...
*/
...
EXCEPTION
    ...
    --WHEN OTHER THEN DEMS_OUTPUT.PUT_LINE ('PRIMARY KEY');
END;
/
