...
/*
    *LPAD / RPAD
    ...
    [표현법]
    LAPD/RPAD(STRING, 최종적으로 반환할 문자열의 길이, [덧붙이고자 하는 문자]
...
SELECT CONCAT('가나다', 'AB') FROM DUAL;
...
/*
    [형변환 함수]
    *TO_CHAR : 숫자타입 또는 날짜타입의 값을 문자타입으로 변환
...
SELECT TO_CHAR(SYSDATE, 'MM DD YYYY"년" HH:MI:SS') FROM DUAL;

--사원들의 이름, 입사날짜(~~~~년 ``월 ``일) 조회
...
SELECT TO_CHAR(SYSDATE, 'YYYY'),
       TO_CHAR(SYSDATE, 'YY'),
       TO_CHAR(SYSDATE, 'RRRR'),  -- RR룰이 따로 존재 -> 50년 이상값이 + 100 EX) 1954
...
SELECT TO_DATE(240807) FROM DUAL;   -- 50년 미만은 자동으로 20XX 50년 이상은 19XX

SELECT TO_DATE(020505) FROM DUAL;   --숫자는 0으로 시작할 수 없다
SELECT TO_DATE('020505') FROM DUAL;

SELECT TO_DATE('020505 120500') FROM DUAL; --포멧을 정해줘야 시 분 초 표시가능
...
--*NULLIF(비교대상1, 비교대상2)
--두 값이 일치하면 NULL, 일치하지 않으면 비교대상 반환
...
/*
    [선택함수]
    *DECODE(비교하고자하는대상(컬럼, 연산식, 함수식, 비교값1,결과값1, 비교값2, 결과값2...)
*/
...
--=============================================================
--1. SUM(숫자타입컬럼) : 해당컬럼 값들의 총 합계를 구해서 반환

--근로자 테이블의 전사원의 총 급여를 구해라
SELECT SUM(SALARY)
FROM EMPLOYEE;
...
--5. COUNT(* | 컬럼 | DISTINCT 컬럼) : 해당 조건에 맞는 행의 갯수를 새서 반환
-- COUNT(*) : 조회된 결과에 모든 행의 갯수를 세서 반환
-- COUNT(컬럼) : 재시한 해당 컬럼값이 NULL이 아닌 것만 행의 수를 세서 반환
-- COUTN(DISTINCT 컬럼) : 해당 컬럼값의 중복제거 후 행의 갯수를 세서 반환
