    ...
    1. '%' : 포함문자 검색(0글자 이상 전부 조회)
     EX) 비교할 대상 컬럼 LIKE '문자%' : 비교할 대상 컬럼값 중에서 해당문자로 시작하는 값들만 가져온다.
         비교할 대상 컬럼 LIKE '%문자' : 비교할 대상 컬럼값 중에서 해당문자로 끝나는 값들만 가져온다.
          비교할 대상 컬럼 LIKE '%문자%' : 비교할 대상 컬럼값 중에서 해당문자가 포함된 값들만 가져온다.
          
    2, '_' : 1글자를 대체하는 검색
     EX) 비교할 대상 컬럼 LIKE '_문자' : 비교할대상컬럼값 문자앞에 아무글자나 한글자가 있는 값을 조회
         비교할 대상 컬럼 LIKE '문자_' : 비교할대상컬럼값 문자뒤에 아무글자나 한글자가 있는 값을 조회
         비교할 대상 컬럼 LIKE '문자_' : 비교할대상컬럼값 문자앞뒤에 아무글자나 한글자가 있는 값을 조회

         비교할 대상 컬럼 LIKE '__문자_____' : 내가 원하는 형태로 _를 통해서 문자수를 조절할 수 있다.
*/
...
--사원들중 이름에 '하'라는 글자가 포함된 사원의 사원명, 급여, 입사일 조회
SELECT EMP_NAME, SALARY, HIRE_DATE
FROM EMPLOYEE
...
SELECT EMP_NAME, SALARY, HIRE_DATE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '_하%_';

--전화번호의 3번쨰자리가 1인 사원들의 사번, 사원명, 전화번호 조회
SELECT EMP_NAME, EMP_NO, PHONE
FROM EMPLOYEE
...
FROM EMPLOYEE
--WHERE EMAIL LIKE'%'; -> 와일드카드 문자떄문에 정상출력이 되지않음
--와일드카드 문자와 일반문자를 구분해줘야한다.
--데이터값으로 취급하고싶은 와일드카드 문자앞에 나만의 탈출문자를 제시해서 탈출시겨주면된다.
...
-- 4. 급여가 200만원 이상이고 500만원 이하인 사원 중에서 입사일이 '01/01/01'이상이고 보너스를 받지 않은 사원의
...
SELECT EMP_ID, EMP_NAME, SALARY, SALARY * (BONUS + 1) * 12 AS 보너스포함연봉
FROM EMPLOYEE
